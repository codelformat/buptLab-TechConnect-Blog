...
export default function Comment({ comment, onLike ,onEdit,onDelete}) {
    ...
    const handleSave = async () => {
        try {
            const res = await fetch(`/api/comment/editedComment/${comment._d}`, {
                method: 'PUT',
                headers: {
                        ...
                        <div className='flex items-center pt-2 text-xs border-t dark:border-gray-700 max-w-fit gap-2'>
                            ...
                            </p>
                            {
                                currentUser && (currentUser._id === comment.userId || currentUser.isAdmin)
                                    ...
                                    (<>
                                        <button
                                            type='button' className='text-gray-400 hover:text-blue-500' onClick={handleEdit}>
                                            Edit
                                        </button>
                                        <button
                                            type='button' className='text-gray-400 hover:text-red-500' onClick={()=>onDelete(comment._id)}>
                                            Delete
                                        </button>    
                                    </>)
                            }
